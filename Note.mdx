...
11. `cat` - one way of seeing files in linux is good for short files also if you write the name of two file it will combine and print the both
12. `more` - use for large file but the problem is show list of file but when you scroll you can go to the top
13. `less` - install the less to have ablitiy to go up and dow
14. `head -n` - then specifie the number wich mean how many line do you wants to see 
15. `tail -n` - oppsite of head you sepcifie the number and will show the last line of that file
16. `cat file1.txt > file2.txt` - now it's copay all data from file1.txt and create same call file2.txt
by writing `cat file1.txt file2.txt > combine.txt` this will copy and data of file1.txt and file2.txt into combine.txt
also it this `>` is not only for cat it's work in other part like this `echo hello > hello.txt` now we have new file 
call hello.txt that have hello text inside of it.
